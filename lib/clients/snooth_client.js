    ...
    for (var i=0; i<data.length; i++) {
        ...
        if (name && store.lat && store.lng) {
            ...
            if (store.num_wines>0) {
                var winesLink = "<a href='#' ng-click='getWines(" + storeId + ")'>wines</a>" ;
                console.log('link', winesLink)
                sidebar = message + "<br>" + winesLink;
                markers[name]['icon'] = wineIcon;
            } else {
                sidebar = message;
            }  
            markers[name]['message'] = message;
            markers[name]['sidebar'] = sidebar;
        } else {
            // need some geoloc
